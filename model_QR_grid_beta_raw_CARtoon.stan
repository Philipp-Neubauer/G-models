...
model {
  ...
  for (i in 1:N_islands) {
    segment(grid_fx,pos,grids[i]) ~ sparse_car(grid_sig[i], rho[i], segment(W_sparse,ppos,W_n[i]), segment(D_sparse,pos,grids[i]), segment(lambda,pos,grids[i]), grids[i], W_n[i]);
    pos = pos + grids[i];
    ppos = ppos + W_n[i];
  }
