...
```{r echo=T}
res <- inla(mean_species ~ 0 + IWP + TED + INT + 
...
```{r fig.align='center',fig.cap='Model fit from spatial model for Macroalgae',fig.height=12,fig.width=8}
ry <- res$summary.fitted.values$mode-cdata$mean_species
rys <- data.frame(Bearing = cdata$BEARING,
                  Residual = ry,
                  Predicted = res$summary.fitted.values$mode,
...
```{r fig.align='center',fig.cap='Model fit from spatial model for Macroalgae by bearing, data in black, predictions in blue',fig.height=12,fig.width=8}  
...
```{r fig.align='center',fig.cap='Estimated deviation of individual island regression slopes from over-all trend for Macroalgae',fig.height=12,fig.width=8}
iwp <- res$summary.random$island_IWP
ted <- res$summary.random$island_TED
iwpxted <- res$summary.random$island_INT
