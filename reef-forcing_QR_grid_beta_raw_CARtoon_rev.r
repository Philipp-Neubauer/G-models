...
#           pars=c('omean','rho','grid_sig','beta[1,1]',
...
# rys <- data.frame(g = model_data$GRID[model_data$ii_notzero],ry,i=unique(all_data$ISLAND)[model_data$ISLAND[model_data$ii_notzero]],imean=logit(model_data$IMEAN))
# #ggplot(rys) + geom_line(aes(x=g,y=ry)) + facet_wrap(~as.factor(i),scales='free')
# # 
# require(purrr)
# acc <- rys %>% arrange(i,g) %>% split(.$i) %>% map(~acf(.$ry,plot=F,type='cov',lag.max = 100)) %>% map(~with(.,data.frame(lag,acf))) %>% bind_rows(.id='Island')
...
save(fit_CCARtoon_rev,file='CARtoonfit_raw_cca_hhCAR.Rdata')
#' 
#' 
#' 
...
save(fit_CARtoon_rev,file='CARtoonfit_raw_coral_hhCAR.Rdata')
#
# # 
